...
const Content = () => {
    ...
    const [link, setlink] = useState('https://mdf28server.site/api/bascketball/search/player/?limit=16&offset=0')
    ...
    let SearhPl = async () => {
        let response = await fetch(`https://mdf28server.site/api/bascketball/search/player/?search=${nameP}&limit=14&offset=0`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    useEffect(() => {
        if (nameP.length <= 0) {
            let SearhPll = async () => {
                let response = await fetch(`https://mdf28server.site/api/bascketball/search/player/?limit=16&offset=0`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        <>
            <div className={styles.content}>
                <div className={styles.headerr}>
                    <div>
                        <input type='text' onChange={changeHolder} value={nameP} className={styles.Input} placeholder='найти игрока' maxLength={255} /><img src='/mdf28/svg/venok.svg' /><img id={styles.id_1} src='/svg/lupa.svg' />
                    </div>
                </div>
                <div>
                    ...
                    <div>
                        {load1 ? <span className="loader" id="id_00" style={{ transform: 'translateX(22px)' }}>загрузка..</span> : <>{info && info.map((el) => (<div className={styles.info_el} onClick={() => navigate(`/profile/${el.user?.id}`)}><div style={{ backgroundImage: `url(${el.user?.ava})` }} className={styles.ava}></div><p>{el.user?.first_name} {el.user?.last_name} {el.user?.smail && <div style={{ backgroundImage: `url(${el.user?.smail.image})` }} className={styles.smail}></div>}
                            ...
                            {el.is_recognized && <img src='/svg/venok.svg' id={styles.id_2} />} </p><div className={styles.dotas}><span> {el.team?.team_name} </span><p style={{fontSize: '21px', marginTop: '8px'}}>{el.number}</p></div></div>))}</>}
                    </div>
