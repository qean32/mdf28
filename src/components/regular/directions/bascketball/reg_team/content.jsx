...
const Content = () => {
    ...
    let reg = async (e) => {
        ...
        if (dir) {
            ...
            let response = await fetch('http://qean32.beget.tech/api/bascketball/reg/team/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    let regg = async (id) => {
        let response = await fetch(`http://qean32.beget.tech/api/bascketball/update/player_user/${user.user_id}/`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `JWT ${JSON.parse(localStorage.getItem('token')).access}`,
