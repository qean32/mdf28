...
const Content = () => {
    ...
    const [link, setlink] = useState('https://mdf28server.site/api/bascketball/search/team/?limit=14&offset=0')
    ...
    let SearhPl = async () => {
        let response = await fetch(`https://mdf28server.site/api/bascketball/search/team/?search=${nameP}&limit=14&offset=0`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    useEffect(() => {
        if (nameP.length <= 0) {
            let SearhPll = async () => {
                let response = await fetch(`https://mdf28server.site/api/bascketball/search/team/?limit=14&offset=0`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    },
