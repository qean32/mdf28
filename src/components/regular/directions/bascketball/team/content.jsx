        ...
        <>
            <div className={styles.content}>
                <div className={styles.back} style={{ backgroundImage: `url(${data.background})` }}>
                    {id && id == data.director?.id && <img src="/mdf28/svg/menu_b.svg" style={{ position: 'absolute', right: '12px', top: '10px', cursor: 'pointer' }} />}
                    {id && id == data.director?.id && <div className={id ? styles.leave : styles.leavego}>
                        <div><p>редактировать</p><img src="/mdf28/svg/pencil.svg" style={{ marginLeft: '5px' }} /></div>
                        <div onClick={id}><p style={{ transform: 'translateY(-2px)' }}>выйти</p><img src="/mdf28/svg/logout.svg" style={{ marginLeft: '5px' }} /></div>
                    </div>}
                </div>
