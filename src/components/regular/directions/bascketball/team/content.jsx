        ...
        <>
            ...
            {og.length > 0 && <div className={styles.content}>
                ...
                {Array.isArray(og) && og.map((el) => <div className={styles.info_el} onClick={() => navigate(`/profile/${el.user?.id}`)}>
                    ...
                    </p><div className={styles.dotas}><div className={styles.pos_list}>{el.position?.length > 0 && el.position.map((el) => <div className={styles.pos} style={{ backgroundImage: `url(${el.image_position})` }}></div>)}</div><p style={{fontSize: '21px', margin: '7px'}}>{el.number}</p></div></div>)}
            </div>}
