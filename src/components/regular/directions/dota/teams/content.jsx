...
const Content = () => {
    ...
    const [link, setlink] = useState('https://mdf28server.site/api/dota/search/team/?limit=14&offset=0')
    ...
    let SearhPl = async () => {
        let response = await fetch(`https://mdf28server.site/api/dota/search/team/?search=${nameP}&limit=14&offset=0`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    useEffect(() => {
        if (nameP.length <= 0) {
            let SearhPll = async () => {
                let response = await fetch(`https://mdf28server.site/api/dota/search/team/?limit=14&offset=0`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    },
            ...
            <div className={styles.content}>
                ...
                <div>
                    ...
                    {load1 ? <span className="loader" id="id_00" style={{ transform: 'translateX(22px)' }}>загрузка..</span> : <>
                        {info && info.map((el) => (<div className={styles.info_el} onClick={() => navigate(`/dota/team/${el.id}`)}><img src={el.logo} className={styles.ava}></img><p>{el.team_name}
