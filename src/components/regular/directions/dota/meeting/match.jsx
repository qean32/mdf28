...
const Match = ({ el }) => {
    ...
    let regrecord = async () => {
        let response = await fetch(`${host}/api/${direction}/reg/record_stat/`, {
            ...
            body: JSON.stringify({ kill: kill, assist: assist, death: death, damage: dmg, damage_t: dmg_t, heal: heal, gold: gold, user: user_, hero: hero, match: el.id, first_team: first_team_, win: win })
        })
        let data = await response.json()
    }
    return (
        <>
            ...
            <div className={styles.moreInfo}>
                {record.length == 0 ? <><div className={styles.repair}><img src="/svg/repair.svg" alt="" style={{ height: '140px' }} /><p>информация отсутствует</p></div></>
                    :
                    <><div>
                        <div className={styles.list}><p>герой</p> <p style={{ marginLeft: '55px' }}>пользователь</p> <p style={{ marginLeft: '55px' }}>У/С/П</p> <p style={{ marginLeft: '45px' }}>dmg/heal/dmg_t</p><p style={{ marginLeft: '45px' }}>предметы</p></div>
                ...
                {user?.is_org ? <div className={styles.record_reg}>
                    ...
                    <input type="number" onChange={(e) => setheal(e.target.value)} placeholder="heal" id="" />
                    <input type="number" onChange={(e) => setdmg_t(e.target.value)} placeholder="dmg_t" id="" />
                    <input type="number" onChange={(e) => setgold(e.target.value)} placeholder="gold" id="" />
                    <input type="number" onChange={(e) => sethero(e.target.value)} placeholder="hero" id="" />
