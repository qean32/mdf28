...
const Content = () => {
    ...
    const [link, setlink] = useState('http://qean32.beget.tech/api/dota/search/player/?limit=16&offset=0')
    ...
    let SearhPl = async () => {
        let response = await fetch(`http://qean32.beget.tech/api/dota/search/player/?search=${nameP}&limit=14&offset=0`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    useEffect(() => {
        if (nameP.length <= 0) {
            let SearhPll = async () => {
                let response = await fetch(`http://qean32.beget.tech/api/dota/search/player/?limit=16&offset=0`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    },
