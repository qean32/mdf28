        ...
        <>
            <div className={styles.content}>
                ...
                <div className={styles.all}>
                    ...
                    {data1.map((el) => <div key={el.id} className={styles.cash}><p style={{ width: "90px" }}>{el.price} ₽</p><p style={{ width: "415px" }}>{el.content}</p><p style={{ width: '110px', fontSize: '16px' }}>{el.created_at}</p><p style={{ width: '60px' }}>{el.direction?.direction_name}</p></div>)}
                </div>
            </div>
        </>
    );
