        ...
        <>
            <div style={zxc ? {} : { display: 'flex' }}>
                {follows.length != 0 ? <div className={styles.content}>
                    ...
                    {follows && follows.map((el) => (<div key={el.by.id} className={styles.info_el} onClick={() => navigate(`/profile/${el.by.id}`)}><div style={{ backgroundImage: `url(${el.by.ava})` }} className={styles.ava}></div><p>{el.by.first_name} {el.by.last_name} {el.by.smail && <div style={{ backgroundImage: `url(${el.by.smail?.image})` }} className={styles.smail}></div>}
                ...
                {followsUser.length != 0 ? <div className={styles.content}>
                    <p>подписчики</p>
                    {followsUser && followsUser.map((el) => (<div key={el.for_r.id} className={styles.info_el} onClick={() => navigate(`/profile/${el.for_r.id}`)}><div style={{ backgroundImage: `url(${el.for_r.ava})` }} className={styles.ava}></div><p>{el.for_r.first_name} {el.for_r.last_name} {el.for_r.smail && <div style={{ backgroundImage: `url(${el.for_r.smail?.image})` }} className={styles.smail}></div>}
