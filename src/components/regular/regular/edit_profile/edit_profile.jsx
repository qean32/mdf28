...
const Edit_Profile_ = ({host}) => {
    ...
    useEffect(() => {
        SearhUser()
        SearhPlayer(1,setdota)
        SearhPlayer(3, setcs)
        SearhPlayer(4, setbascketball)
    }, [])
    const [dota,setdota] = useState(false)
    const [cs,setcs] = useState(false)
    const [bascketball,setbascketball] = useState(false)
    let SearhPlayer = async (direction,set) => {
        let response = await fetch(`${host}/api/unification/search/player/?user=${user?.user_id}&direction=${direction}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    let UpdatePlayer = async (direction) => {
        let response = await fetch(`${host}/api/unification/update/player_user/${direction}/${user.user_id}/`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `JWT ${JSON.parse(localStorage.getItem('token')).access}`,
    ...
    let updatePlayerDOTA = async (direction) => {
        let response = await fetch(`${host}/api/unification/update/player_user/${direction}/${user.user_id}/`, {
            ...
            body: JSON.stringify({ mmr: pts, rank: rankdota })
        })
        let data = await response.json()
        location.reload()
    }
    let updatePlayerCS = async (direction) => {
        let response = await fetch(`${host}/api/unification/update/player_user/${direction}/${user.user_id}/`, {
            ...
            body: JSON.stringify({ mmr: elo, rank: rankcs })
        })
        let data = await response.json()
        location.reload()
    }
            ...
            <div className={styles.content}>
                ...
                <form className={styles.form} onSubmit={(e) => Update(e)}>
                    ...
                    <div style={{ marginTop: '30px' }} >
                        <div><p style={{ transform: 'translateY(10px)' }}>аватарка</p><input type="file" accept='.png,.jpg,.jpeg' onChange={(e) => ChangeFile(e,setava)} alt="" style={{ background: "none", width: '200px' }} /></div>
                        <div><p style={{ transform: 'translateY(10px)' }}>фон профиля</p><input type="file" src="" accept='.png,.jpg,.jpeg' onChange={(e) => ChangeFile(e,setbck)} alt="" style={{ background: "none", width: '200px' }} /></div>
                    </div>
