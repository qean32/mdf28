import styles from './reg_player.module.css'
import { useNavigate } from 'react-router-dom';
import { useContext, useEffect, useState } from 'react';
import context from '../../../../../connections/context';

const Reg_Player = ({ direction, host, str_direction }) => {
    ...
    let Registration = async () => {
        let response = await fetch(`${host}/api/unification/reg/player/${str_direction}/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `JWT ${JSON.parse(localStorage.getItem('token')).access}`,
            },
            body: JSON.stringify({ user: user.user_id, name: ava.first_name + ' ' + ava.last_name, mmr: pts, rank: rankid })
        })
        let data = await response.json()
        if (user?.user_id) {
        ...
        <>
            {view ? <> <div className={styles.content}>
                ...
                <div className={styles.mmr}>
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: '20px' }}>
                            {id_ && <div className='more' style={{ marginTop: '30px' }} onClick={() => Registration()}><p>стать игроком</p></div>}
                ...
                <div className={styles.content} style={{ width: '700px' }}>
                    <img src="/png/steam_id.png" style={{ height: '450px', borderRadius: '15px' }} alt="" />
                </div>
            </>
