...
const Input_pass = ({isValidclass,goPass,viewPassclass_open,viewPassclass,proverka,setline,viewPass,value,validtext,defPclass,lineclass,validline,label,id,namee }) => {
    return ( 
        ...
        <img src='/mdf28/svg/lock-open-solid.svg' onClick={goPass} className={viewPassclass_open} />
        <img src='/mdf28/svg/look_1.svg' onClick={goPass} className={viewPassclass} />
        <input maxLength={15} onChange={proverka} onFocus={() => setline(true)} onBlur={proverka} type={viewPass ? "text" : "password"} name={namee} id={id} value={value} style={validtext} />
        <label htmlFor={id}>
